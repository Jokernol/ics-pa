...
static inline def_DHelper(J) {
  word_t simm = (s->isa.instr.j.simm20 << 20) + (s->isa.instr.j.imm19_12 << 12) + (s->isa.instr.j.imm11 << 11) + (s->isa.instr.j.imm10_1 << 1);
  print_asm(str(hello) "%c %x", suffix_char(id_dest->width), simm);
